...
bool RMGOpticalDetector::ProcessHits(G4Step* step, G4TouchableHistory* /*history*/) {
  ...
  hit->photon_position = step->GetPostStepPoint()->GetPosition();
  hit->photon_momentum_direction = step->GetPostStepPoint()->GetMomentumDirection();

  // Erhalte TrackInformation
  ...
  if (userInfo) {
    ...
    if (nonConstTrackInfo) {
      ...
      lphoton_gamma_kinetic_energy = nonConstTrackInfo->GetPhotonGammaKineticEnergy();
    }
  }
